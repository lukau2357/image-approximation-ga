...
class PopulationPolyRBGAlpha(Population):
    def __init__(self, number_of_vertices : int, number_of_polygons : int, population_size : int,
        ...
        self.feature_space_dim = number_of_polygons * (2 * number_of_vertices + 4)
    ...
    def crossover_uniform(self, *args):
        ...
        for i in range(self.number_of_polygons):
            ...
            c1[i, 2 * self.number_of_vertices : 2 * self.number_of_vertices + 3] = x[i, 2 * self.number_of_vertices : 2 * self.number_of_vertices + 3].copy() if cx_c == 0 else y[i, 2 * self.number_of_vertices : 2 * self.number_of_vertices + 3].copy()
            c2[i, 2 * self.number_of_vertices : 2 * self.number_of_vertices + 3] = x[i, 2 * self.number_of_vertices : 2 * self.number_of_vertices + 3].copy() if cy_c == 0 else y[i, 2 * self.number_of_vertices : 2 * self.number_of_vertices + 3].copy()

        betas = self.rand.uniform(size = (self.number_of_polygons))
        c1[:, 2 * self.number_of_vertices + 3] = betas * x[:, 2 * self.number_of_vertices + 3] + (1 - betas) * y[:, 2 * self.number_of_vertices + 3]
        c2[:, 2 * self.number_of_vertices + 3] = betas * y[:, 2 * self.number_of_vertices + 3] + (1 - betas) * x[:, 2 * self.number_of_vertices + 3]
        return c1, c2

    def mutate(self, chromosome : np.ndarray):
        mutation_matrix = self.rand.uniform(size = (self.number_of_polygons, 
                                                        self.number_of_vertices * 2 + 4))
        ...
        mutations_rgb = np.sum(mutation_matrix[:, self.number_of_vertices * 2: self.number_of_vertices * 2 + 3] < self.mutation_rate)
        mutations_alpha = np.sum(mutation_matrix[:, self.number_of_vertices * 2 + 3] < self.mutation_rate)

        if mutations_x > 0:
            chromosome[:, :self.number_of_vertices * 2:2]\
                [mutation_matrix[:, :self.number_of_vertices * 2:2] < self.mutation_rate] = \
                self.rand.integers(0, self.width, size = (mutations_x))

        if mutations_y > 0:
            chromosome[:, 1:self.number_of_vertices * 2:2]\
            [mutation_matrix[:, 1:self.number_of_vertices * 2:2] < self.mutation_rate] = \
            self.rand.integers(0, self.height, size = (mutations_y))

        if mutations_rgb > 0:
            chromosome[:, self.number_of_vertices * 2: self.number_of_vertices * 2 + 3]\
            [mutation_matrix[:, self.number_of_vertices * 2: self.number_of_vertices * 2 + 3] < self.mutation_rate] = \
            self.rand.integers(0, 256, size = (mutations_rgb))

        if mutations_alpha > 0:
            chromosome[:, self.number_of_vertices * 2 + 3]\
                [mutation_matrix[:, self.number_of_vertices * 2 + 3] < self.mutation_rate] = \
                    self.rand.uniform(size = (mutations_alpha))
    ...
    def mutate_generation(self, mutation_threshold):
        ...
        mutations_rgb = np.sum(mutation_tensor[:, :, self.number_of_vertices * 2: self.number_of_vertices * 2  + 3] < self.mutation_rate)
        mutations_alpha = np.sum(mutation_tensor[:, :, self.number_of_vertices * 2 + 3] < self.mutation_rate)
        ...
        if mutations_rgb > 0:
            self.current_generation[mutation_threshold:, :, self.number_of_vertices * 2: self.number_of_vertices * 2  + 3]\
            [mutation_tensor[:, :, self.number_of_vertices * 2: self.number_of_vertices * 2  + 3] < self.mutation_rate] = \
            self.rand.integers(0, 256, size = (mutations_rgb), dtype = np.int32)

        if mutations_alpha > 0:
            self.current_generation[mutation_threshold:, :, self.number_of_vertices * 2 + 3]\
                [mutation_tensor[:, :, self.number_of_vertices * 2 + 3] < self.mutation_rate] = \
                    self.rand.uniform(size = (mutations_alpha))
    ...
    def load(root):
        ...
        res = PopulationPolyRBGAlpha(data["number_of_vertices"], data["number_of_polygons"],
